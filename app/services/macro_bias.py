...
# Legacy aliases (import-time snapshots); ``compute`` uses get_tuning().
...
_MULT_EXTREME_INVERSE = TUNING.mult_extreme_inverse
_MULT_DATA_SHORT = TUNING.mult_data_short
...
def compute(
    ...
    if daily_close is None or len(daily_close) < t.min_daily_bars:
        return MacroOverlay(
            size_mult=t.mult_data_short,
            advice=f"日线数据不足(<{t.min_daily_bars}根),建议减仓 {t.mult_data_short:.0%}",
            macro_dir="数据不足",
            signal_vs_macro="未知",
            deviation_pct=0.0,
    ...
    trending_up = slope_pct >= t.slope_trend_up
    trending_down = slope_pct <= t.slope_trend_down
    ...
    if extreme and not aligned:
        ...
        # 震荡市逆势 — 最差组合,压到 0.5
        size_mult = t.mult_ranging_inverse
