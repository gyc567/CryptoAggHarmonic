...
def main():
    ...
    p.add_argument("--candidates", required=True, help="JSON file with {gen, cluster, candidates[], symbol_set, quarter}")
    p.add_argument("--state-root", default=str(state.DEFAULT_ROOT), help="loop_state root directory")
    p.add_argument("--workers", type=int, default=4, help="ProcessPoolExecutor worker count")
    p.add_argument("--timeout", type=int, default=900, help="Per-candidate subprocess timeout (seconds)")
    ...
    if args.use_maker_checker:
        ...
        if not mc_runner.enabled:
            ...
            logger.warning(
                "MAKER_CHECKER_ENABLED=false but --use-maker-checker was " "passed; runner will short-circuit to M4-only verdicts",
            )
    ...
    if not jobs:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            futures = {pool.submit(_run_one, j): j["candidate_id"] for j in jobs}
    ...
    state.write_state_md(
        state.render_state_md(
            ...
            plateau_count=0,
            next_queue_size=0,
            last_decision=(f"accepted={accepted_count} rejected={rejected_count} " f"errors={error_count}"),
            notes=[
                f"cluster: {cluster}",
